...
QtObject {
    ...
    function selectAll() { select(0, model.count - 1) }	//WORKAROUND: rowCount doesn't update fast enough for some reason, meaning we can't select the last item after adding some :|
    ...
    function __select(select, first, last) {
        ...
        if (first < 0 || last < 0 || first >= model.count || last >= model.count) {	//WORKAROUND: see above - selectAll()
